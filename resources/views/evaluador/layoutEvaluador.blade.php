                            ...
                            <li class="menu-item ml-3">
                                ...
                                <ul class="menu menu-2 {{ request()->is('evaluador/calificar/*')? '':'d-none'}}">
                                    @foreach ($auxsTemsEval as $item)
                                        @if ($tipoConv === 1) 
                                            <li class="menu-item ml-3"><a class="menu-link" href="{{ route('calificarConoc.index',['id' => $item->id, 'tem' => 'todos']) }}">{{ $item->nombre }}</a></li>
                                        @else 
                                            <li class="menu-item ml-3">
                                                <a class="menu-link menu-icono btn-3" href="#">{{ $item->nombre }}</a>
                                                <ul class="menu menu-3">
                                                    <li class="menu-item ml-3"><a class="menu-link" href="{{ route('calificarConoc.index',['id' => $item->id, 'tem' => 'oral']) }}">Examen Oral</a></li>
                                                    <li class="menu-item ml-3"><a class="menu-link" href="{{ route('calificarConoc.index',['id' => $item->id, 'tem' => 'escrito']) }}">Examen Escrito</a></li>
                                                </ul>
                                            </li>
                                        @endif
                                    @endforeach
